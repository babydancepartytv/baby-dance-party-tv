...
async function syncFromApi(apiKey, channelId) {
  ...
  setSetting('youtube_channel_title', ch.items[0].snippet.title || '');
  if (ch.items[0].snippet.thumbnails) {
    const t = ch.items[0].snippet.thumbnails;
    setSetting('youtube_channel_avatar', (t.medium || t.default || {}).url || '');
  }
  ...
  try {
    ...
    if (/playlistNotFound/.test(e.message)) {
      ...
      setSetting('last_sync_status', 'ok (channel has no videos yet — upload the first video and sync again)');
      return 0;
    }
    throw e;
  }
  ...
  upsertVideos(batch);
  setSetting('last_sync_at', new Date().toISOString());
  setSetting('last_sync_status', 'ok (' + batch.length + ' videos via API)');
  return batch.length;
}
...
async function syncFromRss(channelId) {
  ...
  upsertVideos(videos);
  setSetting('last_sync_at', new Date().toISOString());
  setSetting('last_sync_status', 'ok (' + videos.length + ' videos via RSS fallback)');
...
function upsertVideos(videos) {
  ...
  for (const v of videos) {
    stmt.run(v.video_id, v.title, slugify(v.title) + '-' + v.video_id.slice(-6),
...
async function syncNow() {
  ...
  let channelId = getSetting('youtube_channel_id');
  try {
    if (apiKey) {
      if (!channelId) {
        channelId = await resolveChannelId(apiKey, getSetting('youtube_channel_handle'));
        setSetting('youtube_channel_id', channelId);
      }
    ...
    setSetting('last_sync_status', 'error: ' + e.message);
    throw e;
  }
}
