...
async function seed() {
  ...
  if (Number(catCount) === 0) {
    ...
    for (const c of cats) {
      await run('INSERT INTO categories (name, slug, description, sort_order) VALUES (?, ?, ?, ?)', c);
    }
  }
  ...
  if (Number(pageCount) === 0) {
    ...
    for (const p of pages) {
      await run('INSERT INTO pages (slug, title, content, show_in_nav) VALUES (?, ?, ?, ?)', p);
    }
  }
  ...
  if (Number(adCount) === 0) {
    ...
    for (const s of slots) {
      await run('INSERT INTO ads (slot, label, code, enabled) VALUES (?, ?, ?, 1)', s);
    }
  }
